...
create_test_app() {
    ...
    local test_app_name="test_megabar_$(date +%Y%m%d_%H%M%S)"
    local test_app_path="../$test_app_name"
    
    if confirm "Create test app at $test_app_path to verify changes?"; then
        cd ..
        
        # Check if create_megabar_app.sh exists
        if [ -f "megabar/create_megabar_app.sh" ]; then
            print_status "Using dedicated MegaBar app creator script..."
            ./megabar/create_megabar_app.sh "$test_app_name"
        else
            print_warning "create_megabar_app.sh not found, falling back to manual creation..."
            rails new "$test_app_name" --skip-git --skip-bundle
            cd "$test_app_name"
        ...
        print_success "Test app created at: $test_app_path"
