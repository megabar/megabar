module MegaBar
  class Textbox < ActiveRecord::Base
    ...
    # Deterministic ID generation for Textboxes
    # ID range: 15000-15999
    def self.deterministic_id(field_display_id)
      # Use field_display_id to create unique identifier
      identifier = "textbox_#{field_display_id}"
      hash = Digest::MD5.hexdigest(identifier)
      base_id = 15000 + (hash.to_i(16) % 1000)
      
      # Check for collisions and increment if needed
      while MegaBar::Textbox.exists?(id: base_id)
        base_id += 1
        break if base_id >= 16000  # Don't overflow into next range
      end
