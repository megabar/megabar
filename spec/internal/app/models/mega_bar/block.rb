module MegaBar
  ...
  class Block < ActiveRecord::Base
    ...
    def make_model_displays
      # Create model displays when the block has a model_id and the attr_accessors are set
      # This happens when:
      # 1. Model creation with make_page option (creates page with model displays)
      # 2. Manual block configuration for specific models
      return unless self.model_id
      
      if (!self.model_id.nil? && !self.model_id.to_s.empty? && Integer(self.model_id) > 0)
        model_name = Model.find(self.model_id).name
        actions = []
        
        # Use instance variables (@) to check the accessor values - these are set during creation
