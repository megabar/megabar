namespace :mega_bar do
  ...
  task :dump_deterministic_seeds, [:mega] => :environment do |t, args|
    ...
    mega_bar_layout_section_ids = MegaBar::LayoutSection.where(id: MegaBar::Layable.where(layout_id: mega_bar_layout_ids).pluck(:layout_section_id)).order(:id).pluck(:id)
